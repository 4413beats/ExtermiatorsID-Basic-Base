...
namespace ExterminatorsID
{
	...
	void MainScript::Initialize()
	{
		...
		g_CustomText->AddText(CONSTEXPR_JOAAT("HUD_JOINING"), "Isn't " ExterminatorsID_NAME " the fucking best?");
		g_CustomText->AddText(CONSTEXPR_JOAAT("HUD_TRANSP"), "Isn't " ExterminatorsID_NAME " the fucking best?");

		g_UiManager->AddSubmenu<RegularSubmenu>("", SubmenuHome, [](RegularSubmenu* sub)
		{
			sub->AddOption<SubOption>("General", nullptr, SubmenuGeneral);
			...
			sub->AddOption<RegularOption>("Spawner", nullptr, []
			{
				// Will work in online 
		...
		g_UiManager->AddSubmenu<RegularSubmenu>("", SubmenuGeneral, [](RegularSubmenu* sub)
		{
			sub->AddOption<SubOption>("", nullptr, SubmenuMovement);
		...
		g_UiManager->AddSubmenu<RegularSubmenu>("", SubmenuTest, [](RegularSubmenu* sub)
		{
			sub->AddOption<RegularOption>("Full Ammo", nullptr, []
			{
				g_Logger->Info("untuk fitur pencet seperti reload amunisi");
			});
			...
			sub->AddOption<NumberOption<std::int64_t>>("Damage Weapon", nullptr, &int64Test, 0, 1000, 10);
